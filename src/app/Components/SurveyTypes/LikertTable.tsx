import LikertTableForm from "@/app/Components/Forms/LikertTableForm";
...
export default function LikertTable({
    onSurveyParams,
    inputErrorsHandler,
    newErrors,
    isInputErrorHandler,
    ...
    const { questionsChangeHandler, formsArray, addForm } =
        useSurveyType<LikertTableQuestion>(onSurveyParams);
    return (
        <>
            {formsArray.map((optionIndex) => (
                <Grid
                    ...
                    key={optionIndex}
                >
                    <LikertTableForm<LikertTableQuestion>
                        id={optionIndex} 
